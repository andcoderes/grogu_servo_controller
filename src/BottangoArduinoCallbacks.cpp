...
namespace Callbacks
{
    ...
    void onEarlyLoop()
    {
        // Null in live USB mode -- avoid null-derefing ESP-NOW handling below.
        if (BottangoCore::commandStreamProvider == nullptr) return;

        static bool bootStarted = false;
        if (!bootStarted) {
            bootStarted = true;
            NeutralPose::start(/*fromMidpoint=*/true, /*targetAnimation=*/-1);
        }

        // Bottango auto-restarts idle once nothing's "in progress," which
        // fights a real Stop -- keep re-asserting stop() every loop while
        // stopped stays true, until a new trigger clears it.
        ...
        if (espNow.consumePendingTrigger(animationIndex)) {
            if (animationIndex >= 0 && animationIndex < GeneratedCodeAnimations::getAnimationCount()) {
                Serial.printf("[%lu] ESP-NOW: trigger consumed in onEarlyLoop, animation %d\n", millis(), animationIndex);
                stopped = false;
                NeutralPose::start(/*fromMidpoint=*/false, animationIndex);
            } else {
                Serial.printf("ESP-NOW: trigger index %d out of range\n", animationIndex);
            }
        }

        if (espNow.consumePendingStop()) {
            Serial.println("Bottango: stopping");
            stopped = true;
            NeutralPose::cancel();
            BottangoCore::commandStreamProvider->stop();
            ArmNudge::stopAll();
        ...
        bool nudgePressed;
        if (espNow.consumePendingNudge(nudgeButton, nudgePressed)) {
            stopped = false;
            NeutralPose::cancel();
            ArmNudge::apply(nudgeButton, nudgePressed);
        }

        // Level-based, not edge-triggered -- runs every tick regardless of
        // whether a new Head packet arrived; throttles/times out internally.
        HeadControl::update();

        // If the ESP-NOW link drops mid-nudge/mid-head-move, freeze in
        // place instead of riding the queued curve unattended. Animations
        // aren't affected -- Bottango's own scheduler drives those.
        static bool wasReceiverConnected = false;
        bool receiverConnected = espNow.isReceiverConnected();
        if (wasReceiverConnected && !receiverConnected && (ArmNudge::isActive() || HeadControl::isActive())) {
            Serial.println("ESP-NOW: receiver link lost mid-manual-control, stopping movement");
            ArmNudge::stopAll();
            HeadControl::stopAll();
        }
        wasReceiverConnected = receiverConnected;
    }
