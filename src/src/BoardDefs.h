...
// grogu: play animations exported from Bottango Studio, no PC needed.
#define USE_CODE_COMMAND_STREAM

// grogu: without this, isOffline() is permanently true and USB live
// control silently does nothing. With it, boots into whichever mode was
// last persisted -- Bottango Desktop's own Live/Export toggle switches
// it. ESP-NOW triggers only apply in Export mode (see onEarlyLoop()).
#define ENABLE_DYNAMIC_ANIMATION_SOURCE_SWITCH

// grogu: not part of a Bottango bridge/peer mesh -- paired 1:1 with our
// own receiver board over a separate protocol (communication/EspNowController).
// Bottango's relay stack would also claim the one global esp_now recv
// callback, so it's disabled here to avoid the collision.
#undef RELAY_SUPPORTED
#undef RELAY_COMS_ESPNOW
#endif
...
namespace NamedBoardStartup
{
    ...
    inline void runNamedBoardStartup()
    {
        ...
        // grogu fix: stock order (digitalWrite then pinMode) gets rejected
        // by this core ("IO 2 is not set as GPIO") and silently dropped.
